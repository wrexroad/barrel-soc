...
if(!$input{"command"}){
   ...
   #start the payload
   `nohup perl updater.pl payload=$input{payload} startdate=$input{startdate} midday=$input{midday} mod40_Offset=$input{mod40_Offset} > $configVals{socNas}/payload$input{payload}/.sysout 2> $configVals{socNas}/payload$input{payload}/.sysout < /dev/null &`;
   print "started!!\n";
