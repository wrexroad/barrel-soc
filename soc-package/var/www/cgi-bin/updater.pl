...
sub mainLoop{
	PAYLOADCYCLE:{
		...
		open LASTREAD, $configVals{'socNas'} . '/payload' . 
         $fileObject{'payload'} . '/.lastRead' or 
         ...
         #compare it to the timestamp of the previous run
			...
			if($currenttime == $fileObject{'lasttime'} and scalar(@filelist) > 0){
		      #There is another file in this date directory
				...
				#now that the file is complete, 
            #copy it to the soc-nas or delete it if its empty
				if($stat[7] > 0){
					unless(
                  -d $configVals{'socNas'} . '/payload' . 
                     $fileObject{'payload'} . '/raw/' . 
                     $fileObject{'currentDate'}
               ){
						mkdir 
                     $configVals{'socNas'} . '/payload' .
                     $fileObject{'payload'} . '/raw/' . 
                     $fileObject{'currentDate'};
					}
					copy(
						$configVals{'mocNas'} . '/payload' . $fileObject{'payload'} . 
                     '/' . $fileObject{'currentDate'} . 
                     '/dat/' . $fileObject{'lastfile'}, 
						$configVals{'socNas'} . "/payload" . $fileObject{'payload'} . 
                     "/raw/" . $fileObject{'currentDate'} . "/" . 
                     $fileObject{'lastfile'}, 
						">"
					);
				}else{
					unlink 
                  $configVals{'mocNas'} . '/payload' . 
            ...
            $currenttime == $fileObject{'lasttime'} and 
            scalar(@filelist) == 0
         ){
				#nothing to do, so check for next days directory
            datechange();
...
sub writetolog{ 
	...
	open LOG, 
      "<" . $configVals{'socNas'} . '/payload' . $fileObject{'payload'} . 
         '/.translog' or print 'Can\'t open log for reading. '.$!."\n";
	while($line=<LOG>){
		push @lines, $line;
	}
	close LOG;
	...
	open LOG, 
      ">" . $configVals{'socNas'} . '/payload' . $fileObject{'payload'} . 
         '/.translog' or print 'Can\'t open log for writing. '.$!."\n";
...
sub copy{
	...
	open INFILE, 
      $infile or print 'Can\'t open '.$infile.' for reading. '.$!."\n";
	open OUTFILE, 
      $mode.$outfile or print 'Can\'t open '.$outfile.' for writing. '.$!."\n";
	binmode INFILE;
	binmode OUTFILE;
...
sub trans{
   ...
   $fileObject{'timeout'} = 0;
	...
	@stat = 
      stat $configVals{'mocNas'} . '/payload' . $fileObject{'payload'} . 
	...
	$fileObject{'fileSize'} = 
      -s $configVals{'mocNas'} . "/payload" . $fileObject{'payload'} . "/" . 
         $fileObject{'currentDate'} . "/dat/" . $fileObject{'fileName'};
	if ($fileObject{'fileSize'} == 0){
		writetolog(
         "File " . $configVals{'mocNas'} . 
         "/payload" . $fileObject{'payload'} . "/" . 
         $fileObject{'currentDate'} . "/dat/" . 
         $fileObject{'fileName'} . " is empty.", 1
      );
		return;
	}
   ...
   unlink 
	...
	writetolog(
      'Filename:' . $configVals{'mocNas'} . "/payload" . 
         $fileObject{'payload'} . "/" . $fileObject{'currentDate'} . "/dat/" . 
         $fileObject{'fileName'} . "\n" .
      ' Size:' . $fileObject{'fileSize'} . ', ' . 
      'Total Frames:' .  
         (
            $fileObject{'noFC'} + 
            $fileObject{'shortFrames'} + 
            $fileObject{'badSums'} + 
            $fileObject{'completedFrames'}
         ) .
...
sub datechange{
	...
	@datelist = 
      getDirListing(
         $configVals{'mocNas'}."/payload".$fileObject{'payload'}."/","dir"
      );
	...
	if (@datelist>1) #there is a new date folder to look in
	{
      ...
      if($stat[7] > 0){
		...
		if(
         ...
         writetolog(
            "Extracted from $fileObject{'startdate'} to $fileObject{'enddate'}".
            "on payload $fileObject{'payload'}", 1
         ) and exit 1;
      }
		...
		@filelist = 
         getDirListing(
            ...
            $configVals{mocNas} . "/payload" . $fileObject{'payload'} . "/" . 
            $fileObject{'currentDate'} . 
            "/dat/) for payload $fileObject{payload}!\nDied:$!";
		
		#update .currentdate
		writeFile(
         $configVals{'socNas'} . '/payload' . $fileObject{'payload'} . 
            '/.currentdate', $fileObject{'currentDate'}
      );
      ...
      #Wait for a bit and start over
		...
		until($filelist[0] eq $fileObject{'fileName'} or scalar(@filelist)==1){
         shift(@filelist);
      }
		$fileObject{'fileName'}=shift(@filelist);
	}
...
sub splitframes{
	...
	open CLEAR, ">" . $configVals{'socNas'} . "/payload" . 
      $fileObject{'payload'}."/.tempsci" or die "Could not clear tempsci! $!\n";
	close CLEAR;
	open CLEAR, 
      ">" . $configVals{'socNas'} . "/payload" . $fileObject{'payload'} . 
         "/.temphouse" or die "Could not clear temphouse! $!\n";
	close CLEAR;

	open RAWDAT, 
      "$filename" or writetolog("Could not open $filename! $! Skipping...",1);
	binmode RAWDAT;
	...
	while (read (RAWDAT, $word, 2)) { #read data file 1 word at a time
		...
		if ($word ne 'eb90'){
			...
			if (scalar @{$frames{$i}}==$configVals{'frameLength'}){ 
            #We have a full frame, process and start over
			...
			elsif(scalar @{$frames{$i}}>$configVals{'frameLength'}){ 
            #Frame is too long, dump and start over
				($i,$shortFrame)=0;
				%frames=();
				$word="";
				...
				if($shortFrame!=0){
               #add up all of the pieces and see if we have a whole frame
					($k,$sum)=0;
					while($k<$i){
                  #add the length of each frame piece plus one for the missing 
                  #syncword
						$sum+=scalar @{$frames{$k}}+1;
						$k++;
					}
					
					if ($sum==$configVals{'frameLength'}){ 
                  #got all the pieces, put a frame together and process							
						$k=0;
						while($k<$i){
                  ...
                  #just reference the frames location
						completeFrame(\@{$frames{0}}); 
						($i,$shortFrame)=0;
						%frames=();
						$completedFrames++;
					}
					elsif($sum>$configVals{'frameLength'}){ 
                  #Frame is too long, check if the newest piece is a whole frame
						if(scalar @{$frames{$i}}==$configVals{'frameLength'}){
                     #good frame length found. Process.
...
sub completeFrame{
	...
	#need to add 0xEB90 into the checksum becasue it is out frame splitting value
   $newData{'calcsum'} = hex(eb90);
   for(my $i=0; $i < 105; $i++){$newData{'calcsum'}+=hex($$frameRef[$i])} 
	...
	#Build a hash of translated frame values. 
   #Values with mod>1 are arrays inside hash elements
	
	#translate frame counter words
	$newData{'version'} = 
      hex($$frameRef[0].$$frameRef[1]) >> 27;
	$newData{'payload'} = 
      (hex($$frameRef[0].$$frameRef[1]) &
   ...
   if($fileObject{'mod40_Offset'}){
	...
	#GPS Data
		...
		if($modIndex{'4'} == 0){
		   ...
		   # Because this is labeled "Ascent Rate": ascent is positive, 
         #   decent is negative
	...
	#Mag Data
		...
		$newData{'bx1'} =
         (hex(substr($$frameRef[5].$$frameRef[6], 0, -2)) - 8388608.0) 
         / 
         83886.070;
		$newData{'by1'} =
		   ((hex(substr($$frameRef[6], -2).$$frameRef[7])) - 8388608.0)
         /
         83886.070;
		$newData{'bz1'} =
		   (hex(substr($$frameRef[8].$$frameRef[9], 0, -2)) - 8388608.0)
         /
         83886.070;
		$newData{'bx2'} =
		   ((hex(substr($$frameRef[9], -2).$$frameRef[10])) - 8388608.0)
         /
         83886.070;
		$newData{'by2'} =
		   (hex(substr($$frameRef[11].$$frameRef[12], 0, -2)) - 8388608.0) 
         / 
         83886.070;
		$newData{'bz2'} =
		   ((hex(substr($$frameRef[12], -2).$$frameRef[13])) - 8388608.0)
         /
         83886.070;
		$newData{'bx3'} =
		   (hex(substr($$frameRef[14].$$frameRef[15], 0, -2)) - 8388608.0)
         /
         83886.070;
		$newData{'by3'} =
		   ((hex(substr($$frameRef[15], -2).$$frameRef[16])) - 8388608.0)
         /
         83886.070;
		$newData{'bz3'} =
		   (hex(substr($$frameRef[17].$$frameRef[18], 0, -2)) - 8388608.0)
         /
         83886.070;
		$newData{'bx4'} =
		   ((hex(substr($$frameRef[18], -2).$$frameRef[19])) - 8388608.0)
         /
         83886.070;
		$newData{'by4'} =
		   (hex(substr($$frameRef[20].$$frameRef[21], 0, -2)) - 8388608.0)
         /
         83886.070;
		$newData{'bz4'} =
		   ((hex(substr($$frameRef[21], -2).$$frameRef[22])) - 8388608.0)
         /
         83886.070;
		$newData{'bx'} =
   ...
   #Light Curve Data
	...
	#Medium Specral Data
	getSpectralData(
      $newData{'frameNumber'},$modIndex{'4'},3,84,12,"medspec",$frameRef
   );
	
	#Slow Spectral Data
	getSpectralData(
      $newData{'frameNumber'},$modIndex{'32'},31,96,8,"slowspec",$frameRef
   );
	...
	#   if there is no time sent for this frame, 
   #   set $newData{gps}[1] to be the last frame's time plus 1
   if($newData{'gps'}[1]){ 
   ...
   #If we got this far, we already know we have a good frame number
	$savedData{'frameNumber'} = $newData{frameNumber};
   ...
   saveFormattedNewValue($newData{'ascentRate'}, "GPS_Ascent_Rate","%.3f");
	...
	saveFormattedNewValue(
      $newData{'hk'}[$dataTypes{'hk'}{'T00_Scint'}{'subcom'}],'T00_Scint',"%.2f"
   );
	saveFormattedNewValue(
      $newData{'hk'}[$dataTypes{'hk'}{'T01_Mag'}{'subcom'}],'T01_Mag',"%.2f"
   );
	saveFormattedNewValue(
      $newData{'hk'}[$dataTypes{'hk'}{'T02_ChargeCont'}{'subcom'}],
      'T02_ChargeCont',"%.2f"
   );
	saveFormattedNewValue(
      $newData{'hk'}[$dataTypes{'hk'}{'T03_Battery'}{'subcom'}],
      'T03_Battery',"%.2f"
   );
	saveFormattedNewValue(
      $newData{'hk'}[$dataTypes{'hk'}{'T04_PowerConv'}{'subcom'}],
      'T04_PowerConv',"%.2f"
   );
	saveFormattedNewValue(
      $newData{'hk'}[$dataTypes{'hk'}{'T05_DPU'}{'subcom'}],'T05_DPU',"%.2f"
   );
	saveFormattedNewValue(
      $newData{'hk'}[$dataTypes{'hk'}{'T06_Modem'}{'subcom'}],'T06_Modem',"%.2f"
   );
	saveFormattedNewValue(
      $newData{'hk'}[$dataTypes{'hk'}{'T07_Structure'}{'subcom'}],
      'T07_Structure',"%.2f"
   );
	saveFormattedNewValue(
      $newData{'hk'}[$dataTypes{'hk'}{'T08_Solar1'}{'subcom'}],
      'T08_Solar1',"%.2f"
   );
	saveFormattedNewValue(
	...
	saveFormattedNewValue(
      $newData{'hk'}[$dataTypes{'hk'}{'T13_TermBatt'}{'subcom'}],
      'T13_TermBatt',"%.2f"
   );
	saveFormattedNewValue(
      $newData{'hk'}[$dataTypes{'hk'}{'T14_TermCap'}{'subcom'}],
      'T14_TermCap',"%.2f"
   );
	saveFormattedNewValue(
      $newData{'hk'}[$dataTypes{'hk'}{'T15_CCStat'}{'subcom'}],
      'T15_CCStat',"%.2f"
   );
	
	#save voltage data
	saveFormattedNewValue(
      $newData{'hk'}[$dataTypes{'hk'}{'V00_VoltAtLoad'}{'subcom'}],
      'V00_VoltAtLoad',"%.2f"
   ); 
	saveFormattedNewValue(
      $newData{'hk'}[$dataTypes{'hk'}{'V01_Battery'}{'subcom'}],
      'V01_Battery',"%.2f"
   );
	saveFormattedNewValue(
      $newData{'hk'}[$dataTypes{'hk'}{'V02_Solar1'}{'subcom'}],
      'V02_Solar1',"%.2f"
   );
	saveFormattedNewValue(
      $newData{'hk'}[$dataTypes{'hk'}{'V03_+DPU'}{'subcom'}],'V03_+DPU',"%.2f"
   );
	saveFormattedNewValue(
      $newData{'hk'}[$dataTypes{'hk'}{'V04_+XRayDet'}{'subcom'}],
      'V04_+XRayDet',"%.2f"
   );
	saveFormattedNewValue(
      $newData{'hk'}[$dataTypes{'hk'}{'V05_Modem'}{'subcom'}],'V05_Modem',"%.2f"
   );
	saveFormattedNewValue(
      $newData{'hk'}[$dataTypes{'hk'}{'V06_-XRayDet'}{'subcom'}],
      'V06_-XRayDet',"%.2f"
   );
	saveFormattedNewValue(
      $newData{'hk'}[$dataTypes{'hk'}{'V07_-DPU'}{'subcom'}],'V07_-DPU',"%.2f"
   );
	saveFormattedNewValue(
      $newData{'hk'}[$dataTypes{'hk'}{'V08_Mag'}{'subcom'}],'V08_Mag',"%.2f"
   );
	saveFormattedNewValue(
      $newData{'hk'}[$dataTypes{'hk'}{'V09_Solar2'}{'subcom'}],
      'V09_Solar2',"%.2f"
   );
	saveFormattedNewValue(
      $newData{'hk'}[$dataTypes{'hk'}{'V10_Solar3'}{'subcom'}],
      'V10_Solar3',"%.2f"
   );
	saveFormattedNewValue(
      $newData{'hk'}[$dataTypes{'hk'}{'V11_Solar4'}{'subcom'}],
      'V11_Solar4',"%.2f"
   );
	
	#save current data
	saveFormattedNewValue(
      $newData{'hk'}[$dataTypes{'hk'}{'I00_TotalLoad'}{'subcom'}],
      'I00_TotalLoad',"%.2f"
   );
	saveFormattedNewValue(
      $newData{'hk'}[$dataTypes{'hk'}{'I01_TotalSolar'}{'subcom'}],
      'I01_TotalSolar',"%.2f"
   );
	saveFormattedNewValue(
      $newData{'hk'}[$dataTypes{'hk'}{'I02_Solar1'}{'subcom'}],
      'I02_Solar1',"%.2f"
   );
	saveFormattedNewValue(
      $newData{'hk'}[$dataTypes{'hk'}{'I03_+DPU'}{'subcom'}],
      'I03_+DPU',"%.2f"
   );
	saveFormattedNewValue(
      $newData{'hk'}[$dataTypes{'hk'}{'I04_+XRayDet'}{'subcom'}],
      'I04_+XRayDet',"%.2f"
   );
	saveFormattedNewValue(
      $newData{'hk'}[$dataTypes{'hk'}{'I05_Modem'}{'subcom'}],'I05_Modem',"%.2f"
   );
	saveFormattedNewValue(
      $newData{'hk'}[$dataTypes{'hk'}{'I06_-XRayDet'}{'subcom'}],
      'I06_-XRayDet',"%.2f"
   );
	saveFormattedNewValue(
      $newData{'hk'}[$dataTypes{'hk'}{'I07_-DPU'}{'subcom'}],'I07_-DPU',"%.2f"
   );
	...
	open OUTPUT,
      ">>" . $configVals{'socNas'} . "/payload" . $fileObject{'payload'} .
         "/.datasci" . $fileObject{'currentDate'};
	...
	open OUTPUT,
      ">>" . $configVals{socNas} . "/payload" . $fileObject{payload} . 
         "/.datahouse" . $fileObject{currentDate};
		print OUTPUT $newData{frameNumber} . ",";
		print OUTPUT $newData{gps}[1] . ",";
		
		#print temperature data
		writeFormattedValue(
         $newData{"hk"}[$dataTypes{'hk'}{'T00_Scint'}{'subcom'}],"%.3f"
      );
		writeFormattedValue(
         $newData{"hk"}[$dataTypes{'hk'}{'T01_Mag'}{'subcom'}],"%.3f"
      );
		writeFormattedValue(
         $newData{"hk"}[$dataTypes{'hk'}{'T02_ChargeCont'}{'subcom'}],"%.3f"
      );
		writeFormattedValue(
         $newData{"hk"}[$dataTypes{'hk'}{'T03_Battery'}{'subcom'}],"%.3f"
      );
		writeFormattedValue(
         $newData{"hk"}[$dataTypes{'hk'}{'T04_PowerConv'}{'subcom'}],"%.3f"
      );
		writeFormattedValue(
         $newData{"hk"}[$dataTypes{'hk'}{'T05_DPU'}{'subcom'}],"%.3f"
      );
		writeFormattedValue(
         $newData{"hk"}[$dataTypes{'hk'}{'T06_Modem'}{'subcom'}],"%.3f"
      );
		writeFormattedValue(
         $newData{"hk"}[$dataTypes{'hk'}{'T07_Structure'}{'subcom'}],"%.3f"
      );
		writeFormattedValue(
		...
		writeFormattedValue(
         $newData{"hk"}[$dataTypes{'hk'}{'T13_TermBatt'}{'subcom'}],"%.3f"
      );
		writeFormattedValue(
         $newData{"hk"}[$dataTypes{'hk'}{'T14_TermCap'}{'subcom'}],"%.3f"
      );
		writeFormattedValue(
         $newData{"hk"}[$dataTypes{'hk'}{'T15_CCStat'}{'subcom'}],"%.3f"
      );
	
		#print voltage data
		writeFormattedValue(
         $newData{"hk"}[$dataTypes{'hk'}{'V00_VoltAtLoad'}{'subcom'}],"%.3f"
      ); 
		writeFormattedValue(
         $newData{"hk"}[$dataTypes{'hk'}{'V01_Battery'}{'subcom'}],"%.3f"
      );
		writeFormattedValue(
         $newData{"hk"}[$dataTypes{'hk'}{'V02_Solar1'}{'subcom'}],"%.3f"
      );
		writeFormattedValue(
         $newData{"hk"}[$dataTypes{'hk'}{'V03_+DPU'}{'subcom'}],"%.3f"
      );
		writeFormattedValue(
         $newData{"hk"}[$dataTypes{'hk'}{'V04_+XRayDet'}{'subcom'}],"%.3f"
      );
		writeFormattedValue(
         $newData{"hk"}[$dataTypes{'hk'}{'V05_Modem'}{'subcom'}],"%.3f"
      );
		writeFormattedValue(
         $newData{"hk"}[$dataTypes{'hk'}{'V06_-XRayDet'}{'subcom'}],"%.3f"
      );
		writeFormattedValue(
         $newData{"hk"}[$dataTypes{'hk'}{'V07_-DPU'}{'subcom'}],"%.3f"
      );
		writeFormattedValue(
         $newData{"hk"}[$dataTypes{'hk'}{'V08_Mag'}{'subcom'}],"%.3f"
      );
		writeFormattedValue(
         $newData{"hk"}[$dataTypes{'hk'}{'V09_Solar2'}{'subcom'}],"%.3f"
      );
		writeFormattedValue(
         $newData{"hk"}[$dataTypes{'hk'}{'V10_Solar3'}{'subcom'}],"%.3f"
      );
		writeFormattedValue(
         $newData{"hk"}[$dataTypes{'hk'}{'V11_Solar4'}{'subcom'}],"%.3f"
      );
		
		#print current data
		writeFormattedValue(
         $newData{"hk"}[$dataTypes{'hk'}{'I00_TotalLoad'}{'subcom'}],"%.3f"
      );
		writeFormattedValue(
         $newData{"hk"}[$dataTypes{'hk'}{'I01_TotalSolar'}{'subcom'}],"%.3f"
      );
		writeFormattedValue(
         $newData{"hk"}[$dataTypes{'hk'}{'I02_Solar1'}{'subcom'}],"%.3f"
      );
		writeFormattedValue(
         $newData{"hk"}[$dataTypes{'hk'}{'I03_+DPU'}{'subcom'}],"%.3f"
      );
		writeFormattedValue(
         $newData{"hk"}[$dataTypes{'hk'}{'I04_+XRayDet'}{'subcom'}],"%.3f"
      );
		writeFormattedValue(
         $newData{"hk"}[$dataTypes{'hk'}{'I05_Modem'}{'subcom'}],"%.3f"
      );
		writeFormattedValue(
         $newData{"hk"}[$dataTypes{'hk'}{'I06_-XRayDet'}{'subcom'}],"%.3f"
      );
		writeFormattedValue(
         $newData{"hk"}[$dataTypes{'hk'}{'I07_-DPU'}{'subcom'}],"%.3f"
      );
...
sub getSpectralData{ #gets slow and medium spectra.
	...
	#last frame for this set, check that we have 4 consecutive frames saved
	if($modIndex == $finalMod){		
      #read all the bin values
		...
		#clear the temp file
		open TEMP, ">" . 
         $configVals{'socNas'}.'/payload'.$fileObject{'payload'}.'/.new'.$type;
		close TEMP;
		
		#make sure we have $finalMod+1 lines (need to count the 0 index)
		#and that the first line is the correct number of 
      #frames from the last line
		if(
         $frame_i == ($finalMod + 1) and 
         $frames[0] == ($frames[$finalMod] - $finalMod)
      ){
...
sub getLimits{
   foreach my $type (keys %dataTypes){
      ...
      while(my $line = <LIMITS>){
         #search for section for current payload
         if($line =~ /$fileObject{'payload'}/){ 
            while($line = <LIMITS>) {
               if($line =~ /}/){
               ...
               #Remove trailing newline, remove all whitespace, 
               #remove all quote marks and commas
               $line =~ s/\s//g;
               $line =~ s/"//g;
               $line =~ s/,//g;
...
#creates a fresh set of data files 
#for the new data with a header printed for each data type
sub newDataFiles{
	my $header = "";
	...
	writeFile(
      $configVals{'socNas'} . '/payload' . $fileObject{'payload'}.
         '/.medspec' . $fileObject{'currentDate'}, $header . "\n"
   );
	...
	writeFile(
      $configVals{'socNas'} . '/payload' . $fileObject{'payload'} . 
         '/.slowspec' . $fileObject{'currentDate'}, $header . "\n"
   );
	
	$header = 
      "Frames,Time,GPS_Lat,GPS_Lon,GPS_Alt,LowLevel,PeakDet," . 
      "HighLevel,Interrupt,MAG_X,MAG_Y,MAG_Z,LC1,LC2,LC3,LC4,GPS_PPS\n";
	writeFile(
      $configVals{'socNas'} . '/payload' . $fileObject{'payload'} . 
         '/.datasci' . $fileObject{'currentDate'}, $header
   );
	...
	$header = 
      ...
      "I04_+XRayDet,I05_Modem,I06_-XRayDet,I07_-DPU\n";

	writeFile(
      $configVals{'socNas'} . '/payload' . $fileObject{'payload'} . 
         '/.datahouse'.$fileObject{'currentDate'}, $header
   );
	
	$header = 'Frames,Time,GPS_Lat,GPS_Lon,GPS_Alt,GPS_PPS'."\n";
	writeFile(
      $configVals{'socNas'} . "/payload" . $fileObject{'payload'} .
         "/.gps" . $fileObject{'currentDate'}, $header
   );
	
	$header = 'Frames,Time,LowLevel,PeakDet,HighLevel,Interrupt' . "\n";
	writeFile(
      $configVals{'socNas'} . "/payload" . $fileObject{'payload'} . 
         "/.rc" . $fileObject{'currentDate'}, $header
   );
	
	$header = 
      "Frames,Time," . 
      "MAG_X1,MAG_X2,MAG_X3,MAG_X4,MAG_X_Ave," . 
      "MAG_Y1,MAG_Y2,MAG_Y3,MAG_Y4,MAG_Y_Ave," . 
      "MAG_Z1,MAG_Z2,MAG_Z3,MAG_Z4,MAG_Z_Ave\n";
	writeFile(
      $configVals{'socNas'} . "/payload" . $fileObject{'payload'} . 
         "/.mag" . $fileObject{'currentDate'}, $header
   );
	
	$header = "Frames,Time,LC1,LC2,LC3,LC4\n";
	writeFile(
      $configVals{'socNas'} . "/payload" . $fileObject{'payload'} . 
         "/.lc" . $fileObject{'currentDate'}, $header
   );
	...
	$header = 
      "Frames,Time," . 
      "I00_TotalLoad,I01_TotalSolar,I02_Solar1,I03_+DPU," . 
      "I04_+XRayDet,I05_Modem,I06_-XRayDet,I07_-DPU\n";
	writeFile(
      $configVals{'socNas'} . "/payload" . $fileObject{'payload'} . 
      "/.C" . $fileObject{'currentDate'}, $header
   );
	
	$header = 
      "Frames,Time," . 
      "V00_VoltAtLoad,V01_Battery,V02_Solar1,V03_+DPU,V04_+XRayDet," . 
      "V05_Modem,V06_-XRayDet,V07_-DPU,V08_Mag,V09_Solar2,V10_Solar3," . 
      "V11_Solar4\n";
	writeFile(
      $configVals{'socNas'} . "/payload" . $fileObject{'payload'} . 
         "/.V" . $fileObject{'currentDate'}, $header
   );
...
sub writeValue{
   ...
   # if there is data in this variable, 
   # print it or print "--" to indicate missing value
	if($value ne ""){
		print OUTPUT $value;
	}
	else{
...
sub saveFormattedNewValue{  
   #Add the newly translated values to the %savedData hash
