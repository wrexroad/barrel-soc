var solarnoon = (function(){
   ...
   function getJD()
   {
     ...
     thisMonth = dateObj1.getMonth() + 1;
      ...
      thisYear = Math.floor(parseFloat(thisYear))
     ...
     if ( (isLeapYear(thisYear)) && (thisMonth == 2) ) {
       ...
       if (thisDay > monthList[thisMonth-1].numdays) {
         thisDay = monthList[thisMonth-1].numdays
       }
     }
     if (thisMonth <= 2) {
       thisYear -= 1
     ...
     var A = Math.floor(thisYear/100)
     var B = 2 - A + Math.floor(A/4)
     var JD = Math.floor(365.25*(thisYear + 4716)) + Math.floor(30.6001*(thisMonth+1)) + thisDay + B - 1524.5
     return JD
   }
     ...
     if (isNumber(newTimeUTC)) {
      ...
      ((latitude < -66.4) && ((doy < 83) || (doy > 263))) )
       {   //previous sunrise/next sunset
         ...
         document.getElementById(((rise)? "Sunrise":"Sunset")).value = dayString(jdy,0,3)
   ...
   function balloon_SolarNoon(){
     ...
     var lat = parseFloat(document.getElementById("GPS_Lat").value.substring(0,9));
     var lng = parseFloat(document.getElementById("GPS_Lon").value.substring(0,10));
     var hght = parseFloat(document.getElementById("GPS_Alt").value.substring(0,10));

     //var rise = calcSunriseSet(1, jday, lat, lng, tz, dst)
     //var set  = calcSunriseSet(0, jday, lat, lng, tz, dst)

      var solnoon = calcSolNoon(jday, lng, tz, dst);

   }

   return {
      "calc" : balloon_SolarNoon 
   }
}());
